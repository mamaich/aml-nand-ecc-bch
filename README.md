...
Реализована поддержка NAND микросхем с размером страницы 2176 и 4352 байт.  
...
-skip N - пропустить первые N блоков (блок==4352 или 2176 байта) от начала файла (в выходной файл они будут записаны без изменений). Полезно если правите только в конце дампа и боитесь что утилита что-то поломает в стартовых секторах.  
...
Программа проверена на TC58NVG2S0HTA00 и TC58NVG1S3HTA00 (и их аналогах).   
...
Моя утилита file_combine это делает автоматически.  

Рекомендация по использованию:  
1. Считать дамп программатором  
2. Поправить в дампе ошибки, корректируемые ECC: ./ecc_fix -v [-2 или -4] fixdata dump.bin dump_fixed.bin  
3. Внести свои изменения в dump_fixed.bin, не забываем поправить байты ECC для измененного блока!   
4. Пересчитать ECC: ./ecc_fix -v [-2 или -4] fixecc dump_fixed.bin dump_to_write.bin  
5. Записать dump_to_write.bin программатором  

Подробнее - в статьях моего блога на Dzen.  
